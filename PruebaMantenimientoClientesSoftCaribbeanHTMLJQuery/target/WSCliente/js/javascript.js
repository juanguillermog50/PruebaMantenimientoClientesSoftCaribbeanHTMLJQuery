		...
		.ready(
				function() {
					$
							.ajax(
									{
										url : "http://localhost:8080/WSCliente/getAllClientes",
										type : "GET",
...
function buscar(id) {
	$.ajax({
		url : "http://localhost:8080/WSCliente/getClienteById/" + id,
		type : "GET",
		success : function(data) {
			$("#txtId").val(data.id + "");
...
function eliminar(id) {
	$.ajax({
		url : "http://localhost:8080/WSCliente/deleteCliente/" + id,
		type : "DELETE",
		success : function(data) {
	...
	if (baja > registro) {
		if ($("#txtId").val() == 0) {
			$.ajax({
				...
				url : "http://localhost:8080/WSCliente/addCliente",
				success : function(data) {
			...
			if (confirm("¿Desea sobreescribir al cliente N°:"
				...
				$.ajax({
					...
					type : "PUT",
					data : JSON.stringify(cliente),
					url : "http://localhost:8080/WSCliente/updateCliente",
					success : function(data) {
